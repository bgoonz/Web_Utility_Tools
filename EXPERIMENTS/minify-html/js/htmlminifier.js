...
((global) => {
  ...
  function startTagForHandler(handler) {
    let customStartTagAttrs;

    const startTagAttrs = new RegExp(
      "(?:\\s*[\\w:\\.-]+" +
        "(?:\\s*" +
        "(?:" +
    ...
    if (handler.customAttrSurround) {
      ...
      customStartTagAttrs = new RegExp("((?:" + attrClauses.join("|") + ")*)");
    } else {
    ...
    return new RegExp(
      startTagOpen.source + customStartTagAttrs.source + startTagClose.source
    );
  }
  ...
  const HTMLParser = (global.HTMLParser = (html, handler) => {
    let index;
    let chars;
    let match;
    const stack = [];
    ...
    const startTag = startTagForHandler(handler);
    const attr = attrForHandler(handler);
...
((global) => {
      ...
      (prevTag.substr(0, 1) !== "/" ||
        (prevTag.substr(0, 1) === "/" &&
          (options.collapseInlineTagWhitespace ||
            tags.indexOf(prevTag.substr(1)) === -1)))
    ) {
      str = str.replace(
        /^\s+/,
  ...
  const executableScriptsMimetypes = {
    "text/javascript": 1,
    "text/ecmascript": 1,
    "text/jscript": 1,
  ...
  function isBooleanAttribute(attrName, attrValue) {
    ...
    const attrValueEnumeration =
      enumeratedAttributeValues[attrName.toLowerCase()];
  ...
  function cleanAttributeValue(tag, attrName, attrValue, options, attrs) {
    if (attrValue && isEventAttribute(attrName)) {
      attrValue = trimWhitespace(attrValue)
        ...
        .replace(/\s*;$/, "");
      if (options.minifyJS) {
        const wrappedCode = "(function(){" + attrValue + "})()";
        const minified = minifyJS(wrappedCode, options.minifyJS);
  ...
  const reStartDelimiter = {
    // account for js + html comments (e.g.: //<!--)
    script: /^\s*(?:\/\/)?\s*<!--.*\n?/,
    style: /^\s*<!--\s*/,
  };
  const reEndDelimiter = {
    script: /\s*(?:\/\/)?\s*-->\s*$/,
    style: /\s*-->\s*$/,
  };
  function removeComments(text, tag) {
    return text
      .replace(reStartDelimiter[tag], "")
      .replace(reEndDelimiter[tag], "");
  }

  function isOptionalTag(tag) {
    return /^(?:html|t?body|t?head|tfoot|tr|td|th|dt|dd|option|colgroup|source|track)$/.test(
      tag
    );
  }
  ...
  function normalizeAttribute(
    ...
    const isTerminalOfUnarySlash = unarySlash && index === attrs.length - 1;
    ...
    if (
      (attrValue !== undefined && !options.removeAttributeQuotes) ||
      !canRemoveAttributeQuotes(attrValue, isLast) ||
      isTerminalOfUnarySlash
    ) {
  ...
  function setDefaultTesters(options) {
    const defaultTesters = ["canCollapseWhitespace", "canTrimWhitespace"];

    for (let i = 0, len = defaultTesters.length; i < len; i++) {
      if (!options[defaultTesters[i]]) {
        options[defaultTesters[i]] = () => {
          return false;
        };
      }
    }
  ...
  function minifyURLs(text, options) {
    ...
    try {
      ...
      let __RelateUrl = global.RelateUrl;
  ...
  function minifyJS(text, options) {
    ...
    try {
      // try to get global reference first
      let __UglifyJS = global.UglifyJS;
      ...
      if (__UglifyJS.minify) {
        ...
        const ast = __UglifyJS.parse(text);
        ast.figure_out_scope();

        const compressor = __UglifyJS.Compressor();
        ...
        const stream = __UglifyJS.OutputStream(options.output);
  ...
  function minifyCSS(text, options, inline) {
    ...
    try {
      ...
      if (typeof CleanCSS !== "undefined") {
        ...
        const CleanCSSModule = require("clean-css");
  ...
  function minify(value, options) {
    ...
    value = value.replace(reIgnore, (match, group1) => {
    ...
    function _canCollapseWhitespace(tag, attrs) {
      return (
        canCollapseWhitespace(tag) || options.canCollapseWhitespace(tag, attrs)
      );
    }
    ...
    new HTMLParser(value, {
      ...
      start(tag, attrs, unary, unarySlash) {
        ...
        let openTag = "<" + tag;
        const closeTag =
          (unarySlash && options.keepClosingSlash ? "/" : "") + ">";
        if (attrs.length === 0) {
          openTag += closeTag;
        }
        ...
        const isElementEmpty = currentChars === "" && tag === currentTag;
        if (
      ...
      chars(text, prevTag, nextTag) {
        ...
        if (options.collapseWhitespace) {
          if (!stackNoTrimWhitespace.length) {
            text =
              (prevTag && prevTag !== "comment") ||
              (nextTag && nextTag !== "comment")
                ? collapseWhitespaceSmart(text, prevTag, nextTag, options)
                : trimWhitespace(text);
          }
          if (!stackNoCollapseWhitespace.length) {
            text = !((prevTag && nextTag) || nextTag === "html")
    ...
    results.push.apply(results, buffer);
    let str = joinResultSegments(results, options);

    const customIgnoreRe = new RegExp(
      "\\s*" + trimWhitespace(uidAttr) + "\\s*",
      "g"
    );

    str = str.replace(customIgnoreRe, () => {
      return ignoredCustomMarkupChunks.shift();
    });
    str = str.replace(/<!-- htmlmin:temp -->/g, () => {
...
((global) => {
  ...
  function isDeprecatedAttribute(tag, attrName) {
    return (
      (attrName === "align" &&
        /^(?:caption|applet|iframe|img|imput|object|legend|table|hr|div|h[1-6]|p)$/.test(
          tag
        )) ||
  ...
  class Lint {
    ...
    _reportRepeatingElement() {
      this.log.push(
        "Found <code>&lt;br></code> sequence. Try replacing it with styling."
      );
    }

    testAttribute(tag, attrName, attrValue) {
      if (isEventAttribute(attrName)) {
        this.log.push(
          'Found <span class="event-attribute">event attribute</span> (<strong>' +
            attrName +
            "</strong>) on <strong><code>&lt;" +
            tag +
            "&gt;</code></strong> element."
        );
        ...
        this.log.push(
          'Found <span class="deprecated-attribute">deprecated</span> <strong>' +
            attrName +
            "</strong> attribute on <strong><code>&lt;" +
            tag +
            "&gt;</code></strong> element."
        );
    ...
    testChars(chars) {
      ...
      if (/(&nbsp;\s*){2,}/.test(chars)) {
        this.log.push(
          "Found repeating <strong><code>&amp;nbsp;</code></strong> sequence. Try replacing it with styling."
        );
      }
    }

    test(tag, attrName, attrValue) {
      this.testElement(tag);
      this.testAttribute(tag, attrName, attrValue);
    }

    populate(writeToElement) {
