((exports, global) => {
  ...
  var AST_Toplevel = DEFNODE(
    ...
    "globals",
    {
      ...
      wrap_enclose(arg_parameter_pairs) {
        var self = this;
        var args = [];
        var parameters = [];
        arg_parameter_pairs.forEach(pair => {
  ...
  var AST_Undefined = DEFNODE(
    "Undefined",
    null,
    { $documentation: "The `undefined` value", value: (() => {})() },
    AST_Atom
  );
  var AST_Hole = DEFNODE(
    "Hole",
    null,
    { $documentation: "A hole in an array", value: (() => {})() },
    AST_Atom
  );
  ...
  function parse($TEXT, options) {
    ...
    var subscripts = (expr, allow_calls) => {
    ...
    var expr_op = (left, min_prec, no_in) => {
  ...
  (undefined => {
    ...
    function do_list(list, tw) {
      return MAP(list, node => {
        return node.transform(tw, true);
      });
    }
    _(AST_Node, noop);
    _(AST_LabeledStatement, (self, tw) => {
      self.label = self.label.transform(tw);
      self.body = self.body.transform(tw);
    });
    _(AST_SimpleStatement, (self, tw) => {
      self.body = self.body.transform(tw);
    });
    _(AST_Block, (self, tw) => {
      self.body = do_list(self.body, tw);
    });
    _(AST_DWLoop, (self, tw) => {
    ...
    _(AST_Definitions, (self, tw) => {
      self.definitions = do_list(self.definitions, tw);
    });
    _(AST_VarDef, (self, tw) => {
      self.name = self.name.transform(tw);
      if (self.value) self.value = self.value.transform(tw);
    });
    _(AST_Lambda, (self, tw) => {
    ...
    _(AST_Binary, (self, tw) => {
      self.left = self.left.transform(tw);
      self.right = self.right.transform(tw);
    });
    _(AST_Conditional, (self, tw) => {
      self.condition = self.condition.transform(tw);
    ...
    _(AST_Object, (self, tw) => {
      self.properties = do_list(self.properties, tw);
    });
    _(AST_ObjectProperty, (self, tw) => {
  ...
  AST_Toplevel.DEFMETHOD("figure_out_scope", function (options) {
    options = defaults(options, { screw_ie8: false, cache: null });
    var self = this;
  ...
  AST_SymbolAccessor.DEFMETHOD("unmangleable", () => {
    return true;
  });
  AST_Label.DEFMETHOD("unmangleable", () => {
    return false;
  });
  ...
  AST_LabelRef.DEFMETHOD("undeclared", () => {
    return false;
  });
  AST_Label.DEFMETHOD("undeclared", () => {
    return false;
  });
  ...
  AST_Toplevel.DEFMETHOD("_default_mangler_options", options => {
  ...
  (() => {
    ...
    PARENS(AST_Function, output => {
      return first_in_statement(output);
    });
    PARENS(AST_Object, output => {
      return first_in_statement(output);
    });
    ...
    DEFPRINT(AST_Directive, (self, output) => {
      output.print_string(self.value, self.quote);
      output.semicolon();
    });
    DEFPRINT(AST_Debugger, (self, output) => {
    ...
    DEFPRINT(AST_Statement, (self, output) => {
      self.body.print(output);
      output.semicolon();
    });
    DEFPRINT(AST_Toplevel, (self, output) => {
      display_body(self.body, true, output);
      output.print("");
    });
    DEFPRINT(AST_LabeledStatement, (self, output) => {
      self.label.print(output);
      output.colon();
      self.body.print(output);
    });
    DEFPRINT(AST_SimpleStatement, (self, output) => {
      self.body.print(output);
      output.semicolon();
    });
    ...
    DEFPRINT(AST_BlockStatement, (self, output) => {
      print_bracketed(self.body, output);
    });
    DEFPRINT(AST_EmptyStatement, (self, output) => {
      output.semicolon();
    });
    DEFPRINT(AST_Do, (self, output) => {
      output.print("do");
      output.space();
    ...
    DEFPRINT(AST_While, (self, output) => {
      output.print("while");
      output.space();
      output.with_parens(() => {
        self.condition.print(output);
    ...
    DEFPRINT(AST_ForIn, (self, output) => {
      output.print("for");
      output.space();
      output.with_parens(() => {
        self.init.print(output);
    ...
    DEFPRINT(AST_Lambda, (self, output) => {
    ...
    DEFPRINT(AST_Return, (self, output) => {
      self._do_print(output, "return");
    });
    DEFPRINT(AST_Throw, (self, output) => {
    ...
    DEFPRINT(AST_Break, (self, output) => {
      self._do_print(output, "break");
    });
    DEFPRINT(AST_Continue, (self, output) => {
    ...
    DEFPRINT(AST_Switch, (self, output) => {
      output.print("switch");
      output.space();
      output.with_parens(() => {
        self.expression.print(output);
    ...
    DEFPRINT(AST_Default, (self, output) => {
      output.print("default:");
      self._do_print_body(output);
    });
    DEFPRINT(AST_Case, (self, output) => {
      output.print("case");
      output.space();
      self.expression.print(output);
      output.print(":");
    ...
    DEFPRINT(AST_Catch, (self, output) => {
      output.print("catch");
      output.space();
      output.with_parens(() => {
    ...
    DEFPRINT(AST_Finally, (self, output) => {
    ...
    DEFPRINT(AST_Var, (self, output) => {
      self._do_print(output, "var");
    });
    DEFPRINT(AST_Const, (self, output) => {
    ...
    DEFPRINT(AST_VarDef, (self, output) => {
      self.name.print(output);
      if (self.value) {
        output.space();
        output.print("=");
    ...
    DEFPRINT(AST_Seq, (self, output) => {
      self._do_print(output);
    });
    DEFPRINT(AST_Dot, (self, output) => {
    ...
    DEFPRINT(AST_Sub, (self, output) => {
      self.expression.print(output);
      output.print("[");
      self.property.print(output);
      output.print("]");
    });
    DEFPRINT(AST_UnaryPrefix, (self, output) => {
    ...
    DEFPRINT(AST_UnaryPostfix, (self, output) => {
      self.expression.print(output);
      output.print(self.operator);
    });
    DEFPRINT(AST_Binary, (self, output) => {
      var op = self.operator;
    ...
    DEFPRINT(AST_Conditional, (self, output) => {
      self.condition.print(output);
      output.space();
      output.print("?");
      output.space();
    ...
    DEFPRINT(AST_Array, (self, output) => {
      output.with_square(() => {
        var a = self.elements,
          len = a.length;
    ...
    DEFPRINT(AST_Object, (self, output) => {
    ...
    DEFPRINT(AST_ObjectKeyVal, (self, output) => {
      var key = self.key;
    ...
    DEFPRINT(AST_ObjectSetter, (self, output) => {
      output.print("set");
      output.space();
      self.key.print(output);
      self.value._do_print(output, true);
    });
    DEFPRINT(AST_ObjectGetter, (self, output) => {
      output.print("get");
      output.space();
      self.key.print(output);
      self.value._do_print(output, true);
    });
    DEFPRINT(AST_Symbol, (self, output) => {
      var def = self.definition();
      output.print_name(def ? def.mangled_name || def.name : self.name);
    });
    DEFPRINT(AST_Undefined, (self, output) => {
      output.print("void 0");
    });
    DEFPRINT(AST_Hole, noop);
    DEFPRINT(AST_Infinity, (self, output) => {
      output.print("Infinity");
    });
    DEFPRINT(AST_NaN, (self, output) => {
      output.print("NaN");
    });
    DEFPRINT(AST_This, (self, output) => {
      output.print("this");
    });
    DEFPRINT(AST_Constant, (self, output) => {
      output.print(self.getValue());
    });
    DEFPRINT(AST_String, (self, output) => {
      output.print_string(self.getValue(), self.quote);
    });
    DEFPRINT(AST_Number, (self, output) => {
    ...
    function regexp_safe_literal(code) {
      return (
        [
          92, 47, 46, 43, 42, 63, 40, 41, 91, 93, 123, 125, 36, 94, 58, 124, 33,
          10, 13, 0, 65279, 8232, 8233,
        ].indexOf(code) < 0
      );
    }
    DEFPRINT(AST_RegExp, (self, output) => {
    ...
    DEFMAP(AST_ObjectSetter, (self, output) => {
      output.add_mapping(self.start, self.key.name);
    });
    DEFMAP(AST_ObjectGetter, (self, output) => {
      output.add_mapping(self.start, self.key.name);
    });
    DEFMAP(AST_ObjectProperty, (self, output) => {
      output.add_mapping(self.start, self.key);
    });
  })();
  ("use strict");

  class Compressor extends TreeTransformer {
    constructor(options, false_by_default) {
      if (!(this instanceof Compressor))
        return new Compressor(options, false_by_default);
      super(this.before, this.after);
  ...
  (() => {
    ...
    function make_node_from_constant(compressor, val, orig) {
      ...
      switch (typeof val) {
        case "string":
          return make_node(AST_String, orig, { value: val }).optimize(
            compressor
          );
        case "number":
        ...
        default:
          if (val === null) {
            return make_node(AST_Null, orig, { value: null }).optimize(
              compressor
            );
          }
          if (val instanceof RegExp) {
            return make_node(AST_RegExp, orig, { value: val }).optimize(
              compressor
            );
          }
          throw new Error(
    ...
    function tighten_body(statements, compressor) {
      ...
      function process_for_angular(statements) {
        ...
        function check_expression(body) {
          if (body && body.args) {
            body.args.forEach((argument, index, array) => {
              var comments = argument.start.comments_before;
              if (
                argument instanceof AST_Lambda &&
                comments.length &&
                has_inject(comments[0])
              ) {
                array[index] = make_array(
                  argument,
                  make_arguments_names_list(argument).concat(argument)
                );
              }
            });
        ...
        return statements.reduce((a, stat) => {
          a.push(stat);
            ...
            if (comments && comments.length > 0) {
              ...
              if (has_inject(last)) {
                if (stat instanceof AST_Defun) {
                  ...
                  compressor.warn(
                    "Unknown statement marked with @ngInject [{file}:{line},{col}]",
                    token
                  );
                }
              }
      ...
      function eliminate_dead_code(statements, compressor) {
        ...
        var self = compressor.self();
        statements = statements.reduce((a, stat) => {
          if (has_quit) {
      ...
      function sequencesize(statements, compressor) {
        ...
        statements.forEach(stat => {
          if (stat instanceof AST_SimpleStatement && seq.length < 2e3)
            seq.push(stat.body);
    ...
    (def => {
      def(AST_Node, compressor => {
        return true;
      });
      def(AST_EmptyStatement, compressor => {
        return false;
      });
      def(AST_Constant, compressor => {
        return false;
      });
      def(AST_This, compressor => {
        return false;
      });
      def(AST_Call, function (compressor) {
      ...
      def(AST_Defun, compressor => {
        return true;
      });
      def(AST_Function, compressor => {
        return false;
      });
      def(AST_Binary, function (compressor) {
        return (
      ...
      def(AST_PropAccess, compressor => {
        return !compressor.option("pure_getters");
      });
      def(AST_Seq, function (compressor) {
        return (
          this.car.has_side_effects(compressor) ||
    ...
    OPT(AST_Directive, (self, compressor) => {
      if (compressor.has_directive(self.value) === "up") {
        return make_node(AST_EmptyStatement, self);
      }
      return self;
    });
    OPT(AST_Debugger, (self, compressor) => {
      if (compressor.option("drop_debugger"))
        return make_node(AST_EmptyStatement, self);
      return self;
    });
    OPT(AST_LabeledStatement, (self, compressor) => {
      if (
    ...
    OPT(AST_Block, (self, compressor) => {
      self.body = tighten_body(self.body, compressor);
      return self;
    });
    OPT(AST_BlockStatement, (self, compressor) => {
      self.body = tighten_body(self.body, compressor);
      ...
      if (
        ...
        var tt = new TreeTransformer(function before(node, descend, in_list) {
          ...
          if (
            ...
            var def = node.definitions.filter(def => {
    ...
    OPT(AST_SimpleStatement, (self, compressor) => {
    ...
    OPT(AST_DWLoop, (self, compressor) => {
        ...
        if (cond.length > 1 && !cond[1]) {
          if (compressor.option("dead_code")) {
            ...
            if (self.init instanceof AST_Statement) {
              ...
              a.push(
                make_node(AST_SimpleStatement, self.init, { body: self.init })
              );
            }
            extract_declarations_from_unreachable_code(
              compressor,
      ...
      if (cond.length > 1) {
        if (cond[1]) {
          ...
          if (compressor.option("dead_code")) {
            ...
            a.push(self.body);
            return make_node(AST_BlockStatement, self, { body: a }).transform(
              compressor
            );
          }
        } else {
        ...
        return make_node(self.body.CTOR, self, {
          value: make_node(AST_Conditional, self, {
            ...
            alternative:
              ...
              make_node(AST_Undefined, self.alternative).optimize(compressor),
          }),
        }).transform(compressor);
      }
      if (
    ...
    OPT(AST_Switch, (self, compressor) => {
      ...
      for (;;) {
        ...
        if (last_branch) {
          ...
          if (
            stat instanceof AST_Break &&
            loop_body(compressor.loopcontrol_target(stat.label)) === self
          )
            ...
            if (
              ...
              return ruined
                ...
                : make_node(AST_BlockStatement, node, {
                    body: node.body.reduce((a, branch) => {
    ...
    OPT(AST_Case, (self, compressor) => {
      self.body = tighten_body(self.body, compressor);
      return self;
    });
    OPT(AST_Try, (self, compressor) => {
      self.body = tighten_body(self.body, compressor);
      return self;
    });
    ...
    AST_Definitions.DEFMETHOD("to_assignments", function () {
      var assignments = this.definitions.reduce((a, def) => {
        if (def.value) {
    ...
    OPT(AST_Definitions, (self, compressor) => {
      if (self.definitions.length == 0)
        return make_node(AST_EmptyStatement, self);
      return self;
    });
    OPT(AST_Function, (self, compressor) => {
    ...
    OPT(AST_Call, (self, compressor) => {
      if (compressor.option("unsafe")) {
        ...
        if (exp instanceof AST_SymbolRef && exp.undeclared()) {
            ...
            case "Function":
              ...
              if (
                ...
                try {
                  ...
                  if (!fun) return self;
                  var args = fun.argnames.map((arg, i) => {
          ...
          exp instanceof AST_Dot &&
          exp.expression instanceof AST_Array &&
          exp.property == "join"
        )
          EXIT: {
            var separator =
              self.args.length == 0
                ? ","
                : self.args[0].evaluate(compressor)[1];
            if (separator == null) break EXIT;
            var elements = exp.expression.elements.reduce((a, el) => {
            ...
            if (separator == "") {
              var first;
              if (
                elements[0][0] instanceof AST_String ||
            ...
            node.expression.expression.elements = elements.map(el => {
              return el[0];
            });
      ...
      if (compressor.option("side_effects")) {
        if (
          ...
          self.args.length == 0 &&
          !AST_Block.prototype.has_side_effects.call(
            self.expression,
            compressor
          )
        ) {
    ...
    OPT(AST_UnaryPostfix, (self, compressor) => {
      return self.lift_sequences(compressor);
    });
    OPT(AST_UnaryPrefix, (self, compressor) => {
    ...
    OPT(AST_Binary, (self, compressor) => {
      ...
      if (commutativeOperators(self.operator)) {
        ...
        if (/^[!=]==?$/.test(self.operator)) {
          if (
            ...
            self.right instanceof AST_Conditional
          ) {
            if (
          ...
          if (
            ...
            self.left instanceof AST_Conditional
          ) {
            if (
    ...
    OPT(AST_SymbolRef, (self, compressor) => {
    ...
    OPT(AST_Infinity, (self, compressor) => {
      return make_node(AST_Binary, self, {
        operator: "/",
    ...
    OPT(AST_Undefined, (self, compressor) => {
      if (compressor.option("unsafe")) {
    ...
    OPT(AST_Assign, (self, compressor) => {
      self = self.lift_sequences(compressor);
      if (
        self.operator == "=" &&
        self.left instanceof AST_SymbolRef &&
    ...
    OPT(AST_Conditional, (self, compressor) => {
      if (!compressor.option("conditionals")) return self;
    ...
    OPT(AST_Boolean, (self, compressor) => {
    ...
    OPT(AST_Return, (self, compressor) => {
  ...
  (() => {
    var MOZ_TO_ME = {
      ...
      ObjectExpression(M) {
        return new AST_Object({
          ...
          properties: M.properties.map(prop => {
            prop.type = "Property";
      ...
      Identifier(M) {
        ...
        return new (
          p.type == "LabeledStatement"
            ...
            ? AST_LabelRef
            : AST_SymbolRef
        )({ start: my_start_token(M), end: my_end_token(M), name: M.name });
      },
    };
    ...
    def_to_moz(AST_Constant, function To_Moz_Literal(M) {
      ...
      if (
        ...
        return {
          ...
          operator: "-",
          prefix: true,
          argument: { type: "Literal", value: -value, raw: M.start.raw },
        };
      }
    ...
    function map(moztype, mytype, propmap) {
      ...
      if (propmap)
        propmap.split(/\s*,\s*/).forEach(prop => {
    ...
    AST_Node.from_mozilla_ast = node => {
