...
$(function () {
  ...
  this.showresults = function (username) {
    ...
    if (!debug) {
      $.getJSON(
        "https://api.github.com/users/" + username + "?callback=?",
        function (data) {
          profile = data.data;
  ...
  if (window.location.hash) self.showresults(window.location.hash.substr(1));
});
...
function updateLanguageGraph() {
    ...
    $("#language-table tbody").append(
      $("<tr>")
        .append(
          $("<td>")
            .text(data[i].name)
            .prepend($("<span>").html("&#9724 "))
            .css("color", colors(data[i].name))
        )
  ...
  if (avatar != 1) {
    ...
    var arcs = vis
      ...
      .attr("data-lang-name", function (d) {
        return d.data.name;
      })
      .attr("data-lang-percent", function (d) {
        return Math.round((d.data.y / totalBytes) * 100) + "%";
      })
      .attr("data-lang-color", function (d) {
