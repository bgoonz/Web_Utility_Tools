...
function getTdValue(rows, index, col) {
  return Number(
    rows.item(index).getElementsByTagName("td").item(col).getAttribute("value")
  );
}
...
function sortTableColumn(table_id, sortColumn) {
  ...
  for (let i = 0; i < rows.length - 1; i++) {
    for (let j = 0; j < rows.length - (i + 1); j++) {
      if (
        getTdValue(rows, j, sortColumn) < getTdValue(rows, j + 1, sortColumn)
      ) {
...
function onRateLimitExceeded() {
  if (!RATE_LIMIT_EXCEEDED) {
    console.warn(
      "[useful-forks] GitHub API rate-limit exceeded. (Since useful-forks sends many requests at once, you might have a lot of `Error Code 403` in your browser Console Logs.)"
    );
...
function allRequestsAreDone() {
  return (
    ONGOING_REQUESTS_COUNTER <= 0 && TOTAL_API_CALLS_COUNTER >= TOTAL_FORKS
  );
}
...
function build_fork_element_html(
  table_body,
  combined_name,
  num_stars,
  num_forks
  ...
  const NEW_ROW = $("<tr>", {
    id: extract_username_from_fork(combined_name),
    class: "useful_forks_repo",
  });
  table_body.append(
    NEW_ROW.append(
      $("<td>")
        .html(getRepoCol(combined_name, false))
        .attr("value", combined_name),
      $("<td>")
...
function add_fork_elements(forkdata_array, user, repo, parentDefaultBranch) {
  ...
  for (let i = 0; i < forkdata_array.length; i++) {
    ...
    const NEW_ROW = build_fork_element_html(
      table_body,
      currFork.full_name,
      currFork.stargazers_count,
      currFork.forks_count
    );
    ...
    const requestPromise = () =>
      octokit.repos.compareCommits({
        ...
        head: `${extract_username_from_fork(currFork.full_name)}:${
          currFork.default_branch
        }`,
      });
    const onSuccess = (responseHeaders, responseData) => {
      if (responseData.total_commits <= AHEAD_COMMITS_FILTER) {
        ...
        NEW_ROW.append(
          ...
          $("<td>", { class: "uf_badge" })
            .html(ahead_badge(responseData.ahead_by))
            .attr("value", responseData.ahead_by),
          $("<td>").html(UF_TABLE_SEPARATOR),
          $("<td>", { class: "uf_badge" })
            .html(behind_badge(responseData.behind_by))
            .attr("value", responseData.behind_by),
          $("<td>")
...
function initial_request(user, repo) {
  ...
  const onSuccess = (responseHeaders, responseData) => {
    ...
    html_txt +=
      UF_TABLE_SEPARATOR + getWatchCol(responseData.subscribers_count);
