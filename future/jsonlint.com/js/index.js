...
class Application {
  constructor() {
    ...
    const paramJSON =
      typeof window.localStorage.onloadJSONParameter !== "undefined"
        ? window.localStorage.onloadJSONParameter
        : query.json;
  ...
  registerEvents() {
    ...
    for (const node of $("[data-ga]")) {
      ...
      ); // eslint-disable-line no-undef, no-loop-func
    }
  ...
  highlightErrorLine(line) {
    if (typeof line === "number") {
      this.errorLine = this.editor.addLineClass(
        line,
        "background",
        "line-error"
      );
