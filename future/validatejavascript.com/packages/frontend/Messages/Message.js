...
export default class Message extends MatreshkaObject {
  renderer =
    '<pre class="result-item"><span class="position" title="Go to the line">{{ line }}:{{ column }}</span> {{ type }} {{ message }} <span class="result-link">(<a href="{{ link }}" target="_blank">{{ ruleId }}</a>)</span> </pre>';

  constructor(data) {
    super(data)
      .calc("type", "severity", (severity) =>
