...
function canvasApp() {
  ...
  function initGraphCalculator() {
    ...
    var y_equals_negative_x_button = document.getElementById(
      "y_equals_negative_x"
    );
    y_equals_negative_x_button.addEventListener(
      "click",
      y_equals_negative_xPressed,
      false
    );
    var y_equals_two_x_button = document.getElementById("y_equals_two_x");
    y_equals_two_x_button.addEventListener(
      "click",
      y_equals_two_xPressed,
      false
    );
    var y_equals_one_half_x_button = document.getElementById(
      "y_equals_one_half_x"
    );
    y_equals_one_half_x_button.addEventListener(
      "click",
      y_equals_one_half_xPressed,
      false
    );
    var reset_grid_button = document.getElementById("reset_grid");
    reset_grid_button.addEventListener(
      "click",
      reset_grid_buttonPressed,
      false
    );
  ...
  function drawGrid() {
    ...
    for (i = 0; i <= can_width; i += can_width / 8) {
      if (i == can_width / 2) {
        // Special handling for horiz/vert axes
    ...
    function do_animation() {
      ...
      if (new_x == theCanvas.width + 5) {
        ...
        status_message.innerHTML =
          "Click a button below the grid to graph an equation";
      }
    }
  }
