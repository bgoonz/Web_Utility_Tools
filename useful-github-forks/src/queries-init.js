...
const UF_MSG_EMPTY_FILTER =
  "All the forks have been filtered out: you can now rest easy!";
...
const UF_MSG_SLOWER =
  "The scan is slowing down (and will stall for a little while) due to the high amount of requests.<br>" +
  "(This is to prevent GitHub API from refusing to respond due to thinking those requests are malicious.)";
const UF_MSG_API_RATE =
  "<b>GitHub API rate-limits exceeded.</b> Consider providing an <b>Access Token</b> if you haven't already (click the button at the top-right).<br>" +
...
const SVG_STAR =
  '<svg class="octicon octicon-star v-align-text-bottom" viewBox="0 0 14 16" width="14" height="16" aria-label="star" role="img"><title>Amount of stars</title><path fill-rule="evenodd" d="M14 6l-4.9-.64L7 1 4.9 5.36 0 6l3.6 3.26L2.67 14 7 11.67 11.33 14l-.93-4.74L14 6z"></path></svg>';
const SVG_EYE =
...
function clearNonErrorMsg() {
  ...
  if (errorMsg !== UF_MSG_ERROR && errorMsg !== UF_MSG_SLOWER) clearMsg();
}
...
$("#useful_forks_inject").append(
  $("<div>", { id: UF_ID_WRAPPER }).append(
    ...
    $("<div>", { id: UF_ID_DATA }).append(
      $("<table>", { id: UF_ID_TABLE }).append($("<tbody>"))
    )
  )
);
